...
echo       pythonnet (required by pywebview on Windows)...
"%PYTHON_EXE%" -m pip --disable-pip-version-check -q install --pre pythonnet
echo       pywebview + pystray + Pillow (desktop wrapper)...
"%PYTHON_EXE%" -m pip --disable-pip-version-check -q install pywebview pystray Pillow
