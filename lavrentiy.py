...
# wim/api/ holds the unified prompt builder (single source of truth for the
# L1-L4 system prompt). Appended so root-level packages (l1_pack, domain_pack)
# resolve first via the bare names already on sys.path above.
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "wim", "api"))
import re
import openai
...
# M-1 (2026-05-16): the epoch primitive guards against bg-thread writes
# completing after switch_profile races in. Every function that may persist
# a profile from a background context (learn_from_sessions, add_trigger_words,
# decay_stale_profile_entries) accepts an `_epoch=None` kwarg and threads it
# through to save_profile. Foreground callers leave _epoch=None and skip the
# gate. Test stubs of save_profile must accept the kwarg — see test_*.py.
...
def reconstruct(raw_text, tone, layer, prof, situation=None,
    ...
    # surfaced from lavrentiy's module-level singletons here so the unified
    # builder remains a pure function. WiM CF passes None for these.
    prior_rej_list = None
    style_examples_list = None
    if layer >= 3 and prof:
        try:
            prior_rej_list = rejection_store.recent()
        except Exception as _e:
            log(f"rejection_store.recent failed: {_e}", "warn")
            prior_rej_list = []
        try:
            style_examples_list = style_examples.recent()
        except Exception as _e:
    ...
    system_prompt = prompt_builder.build_prompt(
        ...
        layer=layer,
        profile=prof,
        situation=situation or current_situation,
        ...
        prior_rejections=prior_rej_list,
        style_examples=style_examples_list,
...
def learn_from_sessions(prof, _epoch=None):
    """Analyze recent raw→output pairs and extract patterns.

    `_epoch` (M-1): bg-thread callers pass `_profile_switch_epoch` from
    launch time; save_profile discards stale writes if a switch raced in."""
    ...
    save_profile(prof, _epoch=_epoch)  # always save — candidate counts changed
...
def decay_stale_profile_entries(prof, _epoch=None):
...
def add_trigger_words(new_triggers, prof, _epoch=None):
    """Merge newly detected trigger words into profile.
    Accepts either a set of words or a dict of {word: disfluency_type}.

    `_epoch` (M-1): when called from a background thread, pass the
    `_profile_switch_epoch` value captured at thread launch. save_profile
    will discard the write if a profile switch has occurred since."""
